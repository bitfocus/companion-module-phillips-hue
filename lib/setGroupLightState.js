...
function setRoom_Switch(id,onoff){
...
function setRoom_Switch_Bri(id,onoff,bri){
  // console.log("setRoomStateUsingState.js -> setRoomState");
  // console.log(id)
  // console.log(onoff);
  if (onoff == 'true') {
    onoff = true
  ...
  console.log(onoff);
   v3.discovery.nupnpSearch()
    ...
    .then(api => {
      ...
      const state = new RoomLightState()
      .on(onoff)
      .bri(bri)
      ;
      return api.groups.setGroupState(id, state);
    })
    .then(result => {
      console.log(`Room state ONOFF change was successful? ${result}`);
    })
